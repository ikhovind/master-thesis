...
\section{What are network sockets}
Anyone perfoming any kind of low-level network programming in a UNIX
environment are bound to have encountered sockets as they are by
far the most prominent abstraction for network communication today.
Sockets exist
...
single transport layer protocol, such as UDP or TCP.
...
This means that the underlying transport protocol is very tightly
...
or other issues. Which transport services are provided by which transport
...
of the application layer being responsible for resolving addresses,
for example through DNS, or being responsible for chosing the specific
transport layer API to use, TAPS provides a unified interface, which
...
\todo{Michael hadde en presentasjon om "problemene" med transportlaget i IN4230, sjekk om den har noe relevant}
...
RFC 9621 describes the architecture of and a set of requirements
for TAPS. \cite{paulyArchitectureRequirementsTransport2025}
A diagram of this architecture can be seen in \cref{fig:tapsArch}.
...
The Transport System Implementation consists of the internal workings
...
\subsubsection{Framers}
In TAPS, a Framer is a concept responsible for briding the TAPS concept
...
A TAPS system should also support adding multiple framers to a preconnection.
In this case, the framers run as a stack, with the last one added running first
for outgoing messages and last for incoming ones. Conceptually, this would
simply introduce more layers into the stack, as shown in \cref{fig:tpsStack}.
...
Not all transport properties are set as a preference, and are
instead specified for example as a boolean, an integer or an enum value.
A full list of transport properties can be found in the Architecture and Requirements for
Transport Services RFC
...
transport properties. These are still useful to understand,
and a selection of the more advanced properties are explained below:
...
\subsection{Candidate Gathering}
When initiating a preconnection, a TAPS system will gather a
set of candidates for establishing a connection. This process
is known as Candidate Gethering. The candidates are based on
the specified local and remote endpoints of the preconnection,
as well as the protocols which are available to use. 
One of the key tenets of TAPS is to support multiple protocols,
it also allows the user to specify both several local and remote endpoints,
in addition to specifying remote endpoints by hostname, which
may resolve into several IP addresses, a single preconnection
can have many possible combinations of protocol, local and 
remote endpoints. Therefore, when intitating a preconnection,
a TAPS implementation needs a way of representing these various
combinations.

RFC9623 conceptualises these combinations as a tree,
with a root node representing the final connection, the
first level of child nodes representing the possible network paths,
which is represented by the possible interfaces given by
any local endpoints specified \cite{brunstromImplementingInterfacesTransport2025}.
\todo{Ask Michael if this is true}
The second level of child nodes represent the possible protocols
which can be used.
The third and final level of child nodes represent any resolved
endpoints. as mentioned before, a single hostname can resolve 
into multiple addresses, both IPv4 and IPv6.
Each leaf node in this tree represents a single candidate
which can be used to establish a connection.

Which branches are possible at each level can be restricted
by the use of selection properties. For example, if the
selection property for reliability is set as "require",
then no branch representing UDP will be featured in the
second level of the final tree. The possible types of interfaces
...
by the final application. This is done through a process known as
Candidate Racing. Candidate Racing is the process of negotiating
...
on the expected amount of time for the handshake \cite{brunstromImplementingInterfacesTransport2025}. 
Since the most desirable candidate will be raced first, it
can be chosen if it connects successfully, limiting the amount
...
Happy Eyeballs, which is described in RFC 86305 is named after the
goal of the algorithm, which is to "reduce user-visible delays on dual-stack hosts"
...
so they share a lot of central features, however, Happy Eyeballs is
primarily concerned with racing IPv4 and IPv6 addresses.
...
While there are many reasons to try to adopt IPv6 as broadly as
possible, there are however still several problems, meaning that fallback to
IPv4 still has to be an option. While clients and servers should prefer using
IPv6, adoption has been slow, and the failure has historically been
observed to be higher than for Ipv4 %\cite{hustongeoffISPColumn}.
This potential brokenness of IPv6 and the consideration that in certain
situations IPv4 can provide a better user experience than IPv6
has led to the development of the Happy Eyeballs algorithm.
There exists several versions of Happy Eyeballs, with the
first version being published in 2012 and describes an algorithm
...
Happy Eyeballs is named after the goal of the algorithm, which
is to improve the experience of the end user of the network.
The basic concept is the same for every version, it
starts two staggered connections, with a slight head start for the IPv6
version, and then uses the connections which first succeeds.
This can also be implemented to race several IP addresses of the
same version in cases where the DNS results contain more than one
result for each version. In this case, IPv6 and IPv4 are interlaced,
avoiding issues with long delays in cases where IPv6 may not work at all.
...
The details of the impelmentation of Candidate Racing in CTaps
is discussed in \cref{sec:candidateRacing}
