...
migration experiments are based on a single run
...
does not reflect actual performance. Simple sequential clients will in these
cases always use the first candidate, regardless of its performance.
...
we can calculate the minimum BDP to carry 70~packets of \qty{1500}{\Bytes}, which is:
...
performed in order to determine behavior under load.
...
relatively constant, or scale with load.
...
sections are sources of overhead.

\subsection{Summary}
To summarize, the experiments show that while CTaps does have a measurable overhead,
it is negligible when compared to a realistic level of network latency for a simple client.
