...
\mono{nf_hook_slow()} in the Linux kernel returns \mono{-EPERM}~\cite{LinuxKernelNfHookSlow}.
...
For the experiment it was desirable for the Picoquic client
...
It demonstrates that a single CTaps client can connect to different servers
...
demonstrate benefits of candidate racing and how it helps find the
endpoint with the smallest delay.
This experiment was modeled after the theoretical
scenario described in~\cref{fig:racingSeqDiagram}.
...
to demonstrate the benefits of connection migration. In this experiment, a \qty{4.6}{\mega\byte} file is transferred over an emulated
...
as it may be after a remote hostname resolution,
...
more contrived. While it still serves to
...
connections never even leave slow start. If some of these connections could be replaced with
separate streams on existing connections, then applications could make
...
with timeout-based migration, and verifying this is left for future work.
...
The experiments show that for simple, I/O-bound applications, the overhead of CTaps will
not be noticeable. In addition, the UDP RTT experiment
also shows that CTaps features a longer tail in worst-case
...
part of the delay seen for the UDP RTT experiment. This could be replaced with reference counting if performance was
...
selection and the error handling itself. This comes with added
simplicity and much easier migration to future protocols.
