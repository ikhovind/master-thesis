...
ubiquitous accross the internet.
...
the lightweight nature of UDP, it is oftern used as a substrate
...
\end{tikzpicture}
\caption{TCP 4-tuple vs QUIC connection after path event.}
...
\begin{figure}[tbp]
    ...
    \begin{lstlisting}
        ...
        +-----------------------------------------------------+
    \end{lstlisting}
    \caption{API model for socket programming \cite{rfc9622}}
...
\begin{figure}[tbp]
    ...
    \begin{lstlisting}
        ...
        +-----------------------------------------------------+
    \end{lstlisting}
    \caption{API model for TAPS \cite{rfc9622}}
...
There are several ways of mitigating this unecessary waiting,
...
program to modelled sequentially, allowing the structure to be more similar
to a normal CPU-bound program, but it requires multithreading, which gives
can be complex in the form of forcing syncrhonoizatoin of shared data structures,
...
whether or not the request suceeded until the callback is invoked.
...
\section{Glib}
GLib \cite{GLibGitLab} is a genreral purpose utility library which provides several key
...
Picoquic features a built-in event loop, but applications can also intergrate it with their
own event loops,
