...
Instead, it uses a Connection ID, which is independently selected by each
...
a destination ID for an outgoing packet. Multiple Connection IDs
are used for several reasons, among them making sure that connections
cannot be linked by outside observers through a single connection ID.

Connection IDs make addressing in QUIC independent of addressing in lower layer
protocols \cite{iyengarQUICUDPBasedMultiplexed2021}. 

TODO skriv mer, er grafen i connection migration feil? Endres connection ID
seg men det er greit siden vi vet at den er vår?

\subsection{Connection Migration}
One of the key advantages of QUIC is that an established connection
can survive changes in the parameters used in the underlying connection,
unlike TCP which relies on these to identify the connection.
This functionality is illustrated in \cref{fig:connectionBeforeAfterMigration}.

Using a connection ID allows the connection to survive a change
in the addressing of the underlying protocol \cite{iyengarQUICUDPBasedMultiplexed2021}.
When an address is changed, each endpoint has to
independently verify that they can communicate using the new address.
Path validation happens using 
\texttt{PATH\_CHALLENGE} and \texttt{PATH\_RESPONSE} stream
frames.

The \texttt{PATH\_CHALLENGE} frame contains unpredictable
...
\node[title] (quictitle) at (7.3, 0.5) {QUIC};
...
% QUIC side
% ═══════════════════════════════════════════════════════════════════════════════
\node[lbl] at (7.3, 0.0) {Connection = Connection ID};
 
% Before
\node[lbl] at (7.3,-0.5) {before migration};
\node[highlight] at (7.3,-1.05) {CID:\;0xABCD1234};
\node[field]     at (7.3,-1.75) {src IP:\;192.0.2.0};
\node[field]     at (7.3,-2.45) {src port:\;54321};
\node[field]     at (7.3,-3.15) {dst IP:\;198.51.100.0};
\node[field]     at (7.3,-3.85) {dst port\;443};
 
% Arrow
\node at (7.3,-4.4) {$\downarrow$ IP change};
 
% After
\node[lbl] at (7.3,-4.85) {after migration};
\node[highlight] at (7.3,-5.55)  {CID:\;0xABCD1234\;{\normalfont\bfseries\textcolor{green!50!black}{unchanged}}};
\node[dead]      at (7.3,-6.25)  {src IP:\;192.0.2.1\;{\normalfont\bfseries\textcolor{red!70}{changed}}};
\node[field]     at (7.3,-6.95)  {src port:\;54321};
\node[field]     at (7.3,-7.65)  {dst IP:\;198.51.100.0};
\node[field]     at (7.3,-8.35)  {dst port:\;4433};
 
\node[draw=green!60!black, fill=green!15, rounded corners, font=\small,
      text=green!50!black] at (7.3,-9.05) {Connection survives \cmark};
 
\end{tikzpicture}
\caption{TCP 4-tuple vs QUIC connection after path event.}
\label{fig:connectionBeforeAfterMigration}
\end{figure}
\todo{Passer dette med hvordan connection IDs fungerer, med source og dest?}
