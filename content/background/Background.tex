...
Unlike TCP, which uses a 4-tuple of source IP, source port, destination IP
and destionation port to uniquely identify a connection,
QUIC uses an unique identifier known as an 
connection ID. Each peer selects a unique ID, which is used
as a destination ID when transferring data from the associated 
peer. \todo{er dette sant}

Packets sent to this peer are identified as
