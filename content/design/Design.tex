...
These are designed to be extensible without introducing breaking API changes.
Applications can set client and server certificates, ALPN values, or other
...
during racing. Although not specified by RFC~9622,
it is natural to assume that in cases where both insecure and secure transport
protocols are viable, any TAPS System would choose to prioritize the
...
be situations where TAPS places a limitation on the security of an application,
but this is more likely to be related to the specific TAPS implementation than
to TAPS in general. For example, CTaps does not currently support keeping key
...
Although TAPS aims to be as secure as possible by default, there are
trade-offs to be made between the default configuration of a TAPS system 
when it comes to feature-richness and security. 
