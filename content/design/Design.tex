...
while exploring how to best expose QUIC functionality.
...
these callbacks largely mirror the event-based API of TAPS.
...
In order for CTaps to be as modular as possible,
...
its pointer. This pattern can be seen in~\cref{lst:using-remote-endpoint}.
...
\multicolumn{1}{l||}{QUIC connection} & Connection Group\\
...
\midrule
\multicolumn{1}{l||}{\mono{STOP_SENDING}\tnote{5}} & \mono{abort()}\\
\multicolumn{1}{l||}{\mono{RESET_STREAM}\tnote{5}} & \mono{abort()}\\
\multicolumn{1}{l||}{\mono{ack_delay_exponent}\tnote{6}} & Connection Property\\ 
\multicolumn{1}{l||}{\mono{max_ack_delay}\tnote{6}} & Connection Property \\ 
\multicolumn{1}{l||}{Unidirectional streams\tnote{7}} & Connection Property \\
\midrule
\multicolumn{2}{l}{\textit{Not available through TAPS API}} \\
\midrule
\multicolumn{1}{l||}{Idle Timeout\tnote{8}} & ---\\
\multicolumn{1}{l||}{Directional \mono{initial_max_streams_*}} & ---\\
\multicolumn{1}{l||}{Stateless reset} & --- \\
\multicolumn{1}{l||}{Connection ID handling} & ---\\
\bottomrule
\end{tabularx}
\begin{tablenotes}
\footnotesize
...
\item[6] Coarse control is available by setting the \mono{connCapacityProfile} Connection Property.
\item[7] TAPS can receive unidirectional streams and set the \mono{canSend} Connection Property automatically,
    but there is no support for marking the underlying stream as unidirectional when cloning.
\item[8] TAPS can set a timeout, but only for \enquote{trying to reliably deliver data to the Remote Endpoint}~\cite[sec. 8.1.3]{rfc9622}.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
Secondly, the echo response is the responsibility of the layer
