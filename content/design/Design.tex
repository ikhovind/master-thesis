...
Development has prioritized the most distinctive internal TAPS features,
...
protocol interface. This is a interface used
to interact with underlying transport protocols, which ensures that CTaps is extensible.
...
for acting as an asynchronous intermediary between the API and the
...
\end{minted}
\caption[TCP close snippet]{
Example of the TCP interface accessing Socket Manager state when closing a Connection.
This allows the TCP interface to remain stateless internally.}
\label{lst:stateless-tcp-example}
\end{listing}

Since several functions within the protocol interfaces
are asynchronous, their communication with the Socket Manager has to be asynchronous.
The Socket Manager features a structure with internal callbacks which are invoked by the protocol
...
An excerpt of the protocol interface vtable can be seen in
...
\end{minted}
\caption{Excerpt from the protocol interface.}
...
void on_connection_ready_increment(ct_connection_t* connection) {
  ...
  \end{minted}
  \caption[Example callback context]
{
...
CTaps passes ownership of created objects to applications even in the cases where
...
with Connection Groups instead of individual Connections it can consistently use and achieve the same
expected behavior irrespective of the underlying protocol in CTaps.
...
The combination of \mono{multipath} Selection Property and the \mono{multipathPolicy} 
Connection Property~\cite[sec. 8.1.7]{rfc9622} allow applications to express multipath preferences,
...
Secondly, the use of UDP options is the responsibility of the layer
...
since insecure protocols have an inherent advantage in candidate racing since they
