...
void ct_socket_manager_listener_closed_cb(ct_socket_manager_t* socket_manager) {
  ...
  {Socket Manager callback invoked by protocol implementation on Connection close, showing how it only closes the underlying socket when the last Connection closes.}
...
void ct_socket_manager_unref(ct_socket_manager_t* socket_manager) {
  ...
  \end{minted}
  \caption{Internal Socket Manager reference counting, ensuring that the Socket Manager remains valid until all associated objects have been freed.}
...
underlying protocol.
This interface is implemented as a struct defining the properties
and function pointers that a protocol implementation must provide.
...
\end{tikzpicture}
  \caption{Diagram of Connection state transitions in CTaps.
      Connections are only passed to applications after they have been
      established and if racing fails then the application will never
...
The Messages passed to the implemented functions are owned by the protocol stack and
the Framer can therefore modify the Messages in place and does not have to allocate new
memory, unless it is extending the length of the Messages.
...
relative ease of implementation. QUIC was
...
Since this workaround requires that CTaps binds to a wildcard socket,
the kernel will pick the source IP address if it is not specified
by the application. This means that when using the \mono{uv_udp_t}
libuv handle, CTaps cannot control the source address of outgoing data. This means
that the source address will not necessarily match the address expected
by Picoquic, and Picoquic can therefore not reason about which paths are
available and which are not. This makes Picoquic unable to perform
connection migration as expected.
...
create a QUIC context and pass any arriving packets to Picoquic.
...
to test the system end-to-end. The setup and teardown of these
ping servers are done through a standardized test fixture, which is used for all
integration tests.
...
which can be done through the \mono{-}\mono{-}\mono{wrap} flag in the GNU linker~\cite{gnuLinkerManual}.
...
can utilize this by replacing their local shared object file, as long as the
...
int ct_perform_dns_lookup(const char* hostname, const char* service,
  ...
  \caption[CTaps DNS lookup function]{Example of using async \texttt{uv\_getaddrinfo()} utility function,
      replacing the synchronous \texttt{getaddrinfo()}.}
...
libuv event loop using a timer handle. CTaps queries Picoquic
