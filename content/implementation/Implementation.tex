...
This section will cover the implementation.

Since a single Socket Manager can be shared between many Connections,
...
single global instance of each protocol interface among multiple Socket Managers.
...
typedef enum {
  ...
  \end{minted}
  \caption[Connection life cycle states]{Connection life cycle states.}
...
\begin{scope}[on background layer]
    ...
    \node[draw=gray!60, dashed, fill=blue!7, rounded corners, inner sep=0.35cm,
          ...
          label={[font=\scriptsize\itshape, gray]above:Visible to applications}] {};
\end{scope}
...
void ct_socket_manager_aborted_connection_cb(ct_connection_t* connection) {
  ...
  {
    Excerpt from Socket Manager callback invoked by a protocol interface on Connection abort. 
...
needs to be generated. These are based on the data passed to the associated Preconnection.
...
the tree is sorted according to the passed Selection Properties. Finally, the leaves of the tree
are extracted into an array of candidates, which can be used for candidate racing.

As mentioned in \cref{sec:design-scope}, Listeners in CTaps only support
listening on a Local Endpoint for a single protocol. The exact combination
of Local Endpoint and protocol is decided through candidate gathering.
This becomes an issue since there is no way for an application to express a preference for TCP over
QUIC, as QUIC fulfills the same Selection Properties as TCP
(the interfaces can be seen in \cref{lst:quic-impl-struct} and \cref{lst:tcp-impl-struct}).
The only way to create a Listener
which accepts TCP is therefore to rely on the ordering of the
supported protocol array in CTaps. When an application
has no preference for a specific protocol, CTaps will fall
back to the internal ordering of protocol candidates as seen
in \cref{lst:supp-prot-arr}.
...
const ct_protocol_impl_t* const ct_supported_protocols[] = {
  ...
  \end{minted}
  \caption[Internal supported protocol array]{Internal supported protocol array.}
  \label{lst:supp-prot-arr}
\end{listing}

This is an implementation detail of CTaps and relies on CTaps
using a stable sorting algorithm when gathering candidates.
Applications would not have to rely on this internal detail if
Listeners in CTaps were able to listen on more than one protocol.
This is discussed further as a piece of future work in~\cref{sec:protocol-ag-listener}.
...
the message queue.
An example displaying the two branches when
applications enqueue a \mono{receive()}-callback can be seen in \cref{lst:recv-msg}.

Sent Messages are not cached in CTaps, but are
immediately passed to either the libuv protocol handle or to Picoquic,
depending on the protocol interface used.
...
int ct_receive_message(ct_connection_t* connection, const ct_receive_callbacks_t* receive_callbacks) {
    ...
    if (!g_queue_is_empty(connection->received_messages)) {
        // Message already available, invoke callback immediately
    ...
    // No message ready, cache the callback until a message arrives
  ...
  \end{minted}
  \caption[CTaps receive Message implementation]
  {
    Code for enqueueing receive callbacks in CTaps,
    showing how CTaps either invokes the callback immediately if a
    Message is already queued, or caches the callback until a Message arrives
  }
  \label{lst:recv-msg}
\end{listing}
...
all Connection termination is treated as asynchronous by CTaps.
Callbacks are sometimes invoked immediately, such as with UDP which does not
require any asynchronous teardown.

\section{Cloning}
CTaps supports cloning through \mono{ct_connection_clone()}.
This allows applications to create a new Connection to the same
...
The Connection Properties that are not shared are the Connection state enum, the Connection
...
If multipath is set to active, then CTaps will probe the remote socket address through all
...
is useful for showcasing connection migration in TAPS,
...
as a workaround is required for CTaps to be able to perform connection
migration properly. Because the Linux kernel only delivers incoming packets
which match the exact IP of the bound socket, CTaps must
bind to a wildcard address in order to be able to use multiple local
addresses. If a Connection can be established from the first Local Endpoint in the
list of gathered candidates, this is likely the address that the winning
Connection in candidate racing will be bound to.
...
Lacking support for binding to multiple sockets was deemed acceptable since a workaround exists.
...
pointers and are expected to be implemented by applications.
...
passes new data to \mono{ct_message_set_content()}, which will
copy the passed data into the message.
...
\end{minted}
\caption[Framer structure definition]{Framer structure definition from ctaps.h.}
...
static int length_prepend_encode(ct_connection_t* connection,
    ...
    ct_message_set_content(message,
        new_content,
        size + 1);
    free(new_content); // Data has been copied
...
static ct_framer_impl_t length_prepend_framer = {
  ...
  \end{minted}
\caption[Framer implementation]{Example Framer implementation from the CTaps framing tests.}
...
UDP is the simplest of the three supported protocols.
It is supported by libuv through the \mono{uv_udp_t} handle~\cite{libuvLibuvUDPHandle}.
Relative protocol complexity is discussed further in~\cref{sec:protocolComplexity}.

Since UDP is connectionless, CTaps maintains
a hash table internally, mapping incoming UDP messages
to Connections. This is one of the roles of the
...
The \mono{ready()}-callback for the associated Connection is invoked immediately.
This does not transfer any data to the Remote Endpoint
...
been tested and the support for this is unclear. This discussed as further work
...
This also means that the lifetimes of TCP sockets are far easier to manage.
A single Connection is always
bound to a single socket, which is never shared. This means that
the associated socket can always be closed when closing a Connection.

\subsection{QUIC} \label{sec:quicImpl}
QUIC is by far the most complex supported protocol in CTaps.
This can be seen through the required number of lines of code in 
\cref{tab:protocolLoc}.

In order to utilize QUIC, applications must pass Security Parameters
to a Preconnection before initiating. If not, then all QUIC attempts will fail synchronously during
candidate racing. CTaps does not feature opportunistic
...
As mentioned previously, Picoquic is a sans-I/O library. This means that CTaps is responsible for
...
Packets sent over a \mono{uv_udp_t} handle therefore use the
...
CTaps cannot control the source address of the outgoing packet,
which may therefore not match what Picoquic expects.
This prevents Picoquic from reasoning about available paths and performing connection migration.

Instead of using the \mono{uv_udp_t} handle for QUIC, CTaps creates
its own UDP socket through the \mono{socket()} system call.
The resulting file descriptor is monitored using a generic
\mono{uv_poll_t} libuv handle. This does not create much extra
complexity, though it does require manually building the message header
...
Due to QUIC's internal stream management, CTaps cannot reliably associate
outgoing QUIC packets with specific Messages. The \mono{sent()} callback therefore signals
that a Message has been handed to Picoquic for transmission, rather than that
...
use a context pointer passed to this callback to find the associated
Listener and invoke the \mono{connection_received()} callback registered by the
application.
...
Applications must configure it through the passed Security Parameters, in order to utilize QUIC.

ALPN is a negotiation between a QUIC server and client and both Listeners and
...
necessary. The consequences of this stagger delay are discussed further in~\cref{sec:single-endpoint-handshake-discussion}.
...
const ct_protocol_impl_t quic_protocol_interface =
  ...
  \end{minted}
  \caption[QUIC protocol interface excerpt]{
Excerpt of the QUIC interface struct. The full implementation can
...
of a protocol, and for applications to state which services they
desire.
...
but utilizing 0-RTT data is optional.
...
\midrule
\multicolumn{1}{l|}{\mono{reliability}}            & \multicolumn{1}{c||}{Require}       & Prohibit      & Require       & Require       \\
\multicolumn{1}{l|}{\mono{preserveMsgBoundaries}}  & \multicolumn{1}{c||}{No Pref.} & Require       & Prohibit      & Prohibit\tnote{1}       \\
\multicolumn{1}{l|}{\mono{perMsgReliability}}      & \multicolumn{1}{c||}{No Pref.} & Prohibit      & Prohibit      & No Pref.        \\
\multicolumn{1}{l|}{\mono{preserveOrder}}          & \multicolumn{1}{c||}{Require}       & Prohibit      & Require       & Require       \\
\multicolumn{1}{l|}{\mono{zeroRttMsg}}             & \multicolumn{1}{c||}{No Pref.} & No Pref. & No Pref. & No Pref. \\
\multicolumn{1}{l|}{\mono{multistreaming}}         & \multicolumn{1}{c||}{Prefer}        & Prohibit      & Prohibit      & No Pref. \\
\multicolumn{1}{l|}{\mono{fullChecksumSend}}       & \multicolumn{1}{c||}{Require}       & Require       & Require       & Require       \\
\multicolumn{1}{l|}{\mono{fullChecksumRecv}}       & \multicolumn{1}{c||}{Require}       & Require       & Require       & Require       \\
\multicolumn{1}{l|}{\mono{congestionControl}}      & \multicolumn{1}{c||}{Require}       & Prohibit      & Require       & Require       \\
\multicolumn{1}{l|}{\mono{keepAlive}}              & \multicolumn{1}{c||}{No Pref.} & No Pref. & No Pref. & No Pref. \\
\multicolumn{1}{l|}{\mono{useTemporaryLocalAddr}}  & \multicolumn{1}{c||}{Runtime dep.}  & No Pref. & No Pref. & No Pref. \\
\multicolumn{1}{l|}{\mono{softErrorNotify}}        & \multicolumn{1}{c||}{No Pref.} & No Pref. & No Pref. & No Pref. \\
\multicolumn{1}{l|}{\mono{activeReadBeforeSend}}   & \multicolumn{1}{c||}{No Pref.} & No Pref. & No Pref. & No Pref. \\
\multicolumn{1}{l|}{\mono{multipath}\tnote{2}}          & \multicolumn{1}{c||}{Runtime dep.}  & Disabled      & Disabled      & No Pref. \\
\multicolumn{1}{l|}{\mono{advertisesAltAddress}\tnote{3}} & \multicolumn{1}{c||}{false}       & false         & false         & false \\
\multicolumn{1}{l|}{\mono{direction}}              & \multicolumn{1}{c||}{Bidir.} & Bidir. & Bidir. & Bidir. \\
\multicolumn{1}{l|}{\mono{interface}}              & \multicolumn{1}{c||}{Empty set}     & Empty set     & Empty set     & Empty set \\
\multicolumn{1}{l|}{\mono{pvd}}                    & \multicolumn{1}{c||}{Empty set}     & Empty set     & Empty set     & Empty set \\
\bottomrule
\end{tabular}
\begin{tablenotes}
    \footnotesize
    \item[1] CTaps does not support the QUIC datagram extension.
    \item[2] CTaps only supports \mono{multipath} in terms of connection migration, but does
      not support using multiple paths at once.
    \item[3] \mono{advertisesAltAddress} is set to a boolean RFC~9622~\cite[sec. 6.2.15]{rfc9622},
...
The protocol interfaces in CTaps differ
...
\centering
\begin{threeparttable}
\caption[Lines of code for CTaps protocol interfaces]{Overview of lines of code (LoC) on a per-protocol interface basis.}
...
using the \mono{CTAPS_BUILD_SHARED} setting in CMake.

The largest downside of dynamically linking against CTaps
is that it requires its users to have CTaps installed on their
systems for the application binary to find the correct symbols at run time.
...
FetchContent_MakeAvailable(CTaps)
    ...
    \caption[Fetching CTaps through FetchContent]
    {
        ...
        A full CMakeLists.txt file for an application
        using CTaps can be seen in \cref{lst:ctaps-cmakelists}.
    }
    \label{lst:ctaps-fetchcontent}
\end{listing}

\section{Test System} \label{testing}
CTaps' test system is built around Google Test~\cite{GoogleTest2025} and
...
CTaps features two types of tests, unit tests and integration tests.
...
integration tests.
All tests are compiled and run with AddressSanitizer~\cite{addressSanitizer}
enabled, in order to detect memory errors and leaks during test execution.
...
symbol conflicts.
These conflicts can be mitigated by wrapping symbols,
...
\end{minted}
\caption[Mocking UDP functions using FFF]{Example test mocking UDP functions using FFF.}
...
Care has been taken to keep dependencies away from the external API.
This means that applications linking against CTaps do not need to additionally
link against CTaps's dependencies. CTaps will still need to be able to find
its dynamically linked dependencies at runtime.
...
to perform all asynchronous interactions with the network. An example of this
...
int ct_perform_dns_lookup(const char* hostname, const char* service,
  ...
  \end{minted}
  \caption[CTaps DNS lookup function]{Example of using the asynchronous \texttt{uv\_getaddrinfo()} utility function,
