...
This raises an issue with creating
Listeners for TCP, since the Selection Properties featured in the QUIC and
TCP implementations (seen in \cref{lst:quic-impl-struct} and \cref{lst:tcp-impl-struct})
do not allow for preferring TCP above QUIC.
The only way to create a Listener
which accepts TCP is therefore to rely on the implicit ordering of the
supported protocol array internally in CTaps. This is possible
since the sorting of candidates is stable.
This array can be seen in \cref{lst:supp-prot-arr}.
...
This is an implementation specific artifact of CTaps, but would be solved
with creating more flexible Listeners able to listen on multiple protocols.
This is discussed further as future work in~\cref{sec:protocol-ag-listener}.
...
int ct_receive_message(ct_connection_t* connection, const ct_receive_callbacks_t* receive_callbacks) {
  ...
  {
    Excerpt of \mono{ct_receive_message()},
...
static ct_framer_impl_t length_prepend_framer = {
  ...
  \end{minted}
\caption[Framer implementation]{Example Framer implementation structure from the CTaps framing tests.}
...
CTaps creates its own UDP socket through the \mono{socket()} system call instead
of relying on the \mono{uv_udp_t} handle for QUIC.
