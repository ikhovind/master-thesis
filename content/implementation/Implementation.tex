...
on the underlying protocol. An example displaying the two branches when
...
outgoing QUIC packets with specific Messages. \mono{sent()} therefore signals
...
\multicolumn{1}{l|}{\mono{preserveMsgBoundaries}}  & \multicolumn{1}{c||}{No Pref.} & Require       & Prohibit      & Require       \\
...
\multicolumn{1}{l|}{\mono{multipath}\tnote{1}}          & \multicolumn{1}{c||}{Runtime dep.}  & Disabled      & Disabled      & No Pref. \\
\multicolumn{1}{l|}{\mono{advertisesAltAddress}\tnote{2}} & \multicolumn{1}{c||}{false}       & false         & false         & false \\
...
\begin{tablenotes}
    \footnotesize
    \item[1] CTaps only supports \mono{multipath} in terms of connection migration, but does
      not support using multiple paths at once.
    \item[2] \mono{advertisesAltAddress} is set to a boolean RFC~9622~\cite[sec. 6.2.15]{rfc9622},
