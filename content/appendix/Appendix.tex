...
configure_file(resources/key.pem resources/key.pem COPYONLY)
    \end{minted}
    \caption[CTaps application CMakeLists.txt]
    {
...
int main() {
    ...
    // Security parameters are required for QUIC, ignored for TCP
    ...
    // Will attempt a QUIC connection, but TCP after a stagger delay
...
\end{minted}
\captionof{listing}[CTaps client]
{
...
\end{minted}
\captionof{listing}[CTaps QUIC server]{A CTaps QUIC server
...
int main() {
    ...
    //
    // When All else is equal (no preferences to sort by)
    // CTaps will choose TCP over QUIC
    ct_preconnection_listen(listener_precon, &listener_callbacks, &connection_callbacks);
...
\end{minted}
\captionof{listing}[CTaps TCP server]{A CTaps TCP server
that pings back any message received.
Taken from the CTaps example project~\cite{ctaps-example-project}.
}
...
\chapter{Extended CTaps Code Listings}\label{app:internal-code-examples}
This appendix presents examples from the CTaps source code that are 
too lengthy to include inline in the main text. The listings cover the generic 
protocol interface (\cref{lst:impl-struct}), its QUIC instantiation 
(\cref{lst:quic-impl-struct}), the libuv timer callback used to drive 
Picoquic (\cref{lst:timer-callback-function}), and the UDP send function 
used to dispatch QUIC packets with source address control 
(\cref{lst:pktinfo-send-packet}).
...
\end{minted}
\caption[Generic protocol interface]{Generic protocol interface used internally in CTaps. The QUIC implementation
of this interface can be seen in \cref{lst:quic-impl-struct}.}
\label{lst:impl-struct}
\end{listing}
...
const ct_protocol_impl_t quic_protocol_interface =
    ...
    \end{minted}
    \caption[QUIC protocol implementation struct]{QUIC protocol implementation struct.}
    ...
    tcp_protocol_interface =
{
    ...
    \end{minted}
    \caption[TCP protocol implementation struct]{TCP protocol implementation struct.}
    \label{lst:tcp-impl-struct}
\end{listing}
...
static int ct_quic_send_packet_with_pktinfo(ct_udp_poll_handle_t* poll_handle,
    ...
    \end{minted}
    \caption[Sending a Message over UDP for QUIC]
    {
