...
\multicolumn{1}{l||}{OS support\tnote{7}}            & W, L, M(?)               & W, L, M(?)      & M                          & L         \\
...
\begin{tablenotes}
    ...
    \item[4] Taken to mean a single connection can use more than one path over a single session,
        either concurrently or through connection migration.
    \item[5] Can be achieved through multipath TCP~\cite{apple_multipathservicetype}, QUIC support is unclear.
    ...
    \item[7] W = Windows, L = Linux, M = macOS.
\end{tablenotes}
\end{threeparttable}
\end{table}
