...
datagrams, meaning unreliable discrete messages. The default protocol
for the \texttt{SOCK\_DGRAM} is therefore UDP. 
Specifying \texttt{SOCK\_STREAM}
means that the socket will be providing a reliable stream of bytes.
The default protocol for the \texttt{SOCK\_STREAM} is TCP.
...
How to write 1000 words about sockets:
\begin{itemize}
    \item What is a socket 
    \item History of sockets
    \item Compile time binding, SOCK\_DGRAM vs SOCK\_STREAM
    \item What do each of these sockets represent
    \item If you wanted to switch to QUIC, what would be required
\end{itemize}

This compile-time binding may not seem like a large problem, as
it reflects the fact that a connection cannot swap protocols
...
The reason for QUIC being built on top of UDP is to avoid issues
stemming from network ossification. A lot of middleboxes and
other networking infrastructure is optimized for TCP and UDP,
and may in some cases refuse to forward packets where the
transport layer protocol is not TCP or UDP. Avoiding similar
issues in the future is one of the reasons for why QUIC
integrates TLS 1.3 as a mandatory part of the protocol. By
obscuring as much as possible of the packet from middleboxes,
