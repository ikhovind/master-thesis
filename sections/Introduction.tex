...
\begin{itemize}
    \item What is a socket \cite[p. 8]{donahooTCPIPSockets2009}
...
In practice, a socket is an abstraction where an application
can send and receive data over a network. In order to reflect
the underlying transport protocols, several types of sockets exist.
In the Linux kernels \texttt{sys/socket.h} file, there exists
5 standard socket types. One of these types is passed as a
parameter when creating a socket, which the most common
variants being \texttt{SOCK\_DGRAM} and \texttt{SOCK\_STREAM}.

\texttt{SOCK\_DGRAM} is used for TODO cont here

As a continuation of this, the implementation of
network sockets can be either connection oriented or
stream-oriented, a setting which is decided at compile time.
This has propagated into network sockets, for which the type of
